...
particionar sentidos/variantes operacionais, unidades indivisíveis, em um
número fixo de lotes que sejam ao mesmo tempo equilibrados e coerentes,
segundo uma função objetivo com quatro componentes de peso igual --
equilíbrio de demanda, equilíbrio de produção quilométrica, coerência
territorial e afinidade funcional. Três metaheurísticas são implementadas e
comparadas sobre essa mesma formulação -- Busca Tabu, Ant Colony
Optimization (ACO) e Particle Swarm Optimization (PSO) com adaptação por
Random Keys --, além de uma heurística gulosa determinística usada como
referência. O protocolo experimental executa cada combinação de três
tamanhos de instância e seis valores do número de lotes $K$ 30 vezes de
forma independente, com seeds compartilhadas entre algoritmos e testes
estatísticos não paramétricos pareados. A Busca Tabu obtém o melhor
desempenho combinado de qualidade e tempo na maioria das combinações
testadas, com o ACO competitivo apenas na maior instância e a um custo de
tempo muito superior. Um estudo adicional de aceleração por GPU mostra que
o PSO se beneficia de fato dessa aceleração, enquanto o ACO não, por razão
estrutural ligada à natureza sequencial de sua construção.
\end{abstract}
...
$N=150$. Em termos de posição relativa entre algoritmos, a qualidade se
degrada com o tamanho da instância sem inversão de ranking geral, com a
...
três instâncias do custo total e de seus quatro componentes
($CV_D$, $CV_P$, $C_T$, $C_A$), por algoritmo. Os cinco componentes crescem,
para os três algoritmos, ao passar de $K=3$ para $K=8$.

\begin{table}[H]
\centering
\caption{Custo total e componentes por algoritmo, $K=3$ e $K=8$ (médias sobre as três instâncias).}
\label{tab:k3-k8}
...
Aumentar o número de lotes de 3 para 8 piora, em média, todos os componentes
medidos para os três algoritmos -- não há, nestes dados, nenhum valor de $K$
que reduza simultaneamente todos os cinco componentes em relação a valores
...
Um estudo adicional, restrito por desenho ao recorte PSO (30 dos 60
cenários planejados, $N=150$, $K=5$), mediu a aceleração obtida ao delegar a
avaliação da função objetivo a uma GPU NVIDIA RTX 3060, mantendo o restante
do algoritmo no host. O PSO obtém aceleração real e mensurável: speedup
...
contra $48{,}73$~s (Seção 5.4). Concentrada exatamente na maior instância, a
exceção sugere que a vantagem da Busca Tabu se concentra em instâncias
pequenas e médias, e que o espaço de busca maior da instância grande favorece
a exploração mais ampla da construção estocástica do ACO -- ao custo de um
tempo de execução muito superior, entre ×20 e ×61 o da Busca Tabu conforme a
combinação, e um fator de crescimento com o tamanho da instância (×55,55 de
$N=20$ para $N=150$) bem acima dos ×22,37 da Busca Tabu e ×24,36 do PSO
(Seção 5.5).
...
por desenho: o recorte medido cobre 30 dos 60 cenários originalmente
planejados, concentrados no PSO, enquanto a medição do ACO ficou restrita a
apenas três cenários oficiais dentro desse mesmo recorte -- cobertura bem
mais limitada, que não permite generalizar a ausência de aceleração do ACO
além do que foi efetivamente medido nesses três cenários. A Busca Tabu, por
sua vez, não foi objeto desse estudo adicional. Um teste específico da
suíte de conformidade GPU adota, além disso, uma régua de tolerância
numérica mais estrita que a régua normativa do projeto -- uma divergência de
